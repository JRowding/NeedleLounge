...
function PageContents({ page, onBookingOpen }: { page: Page; onBookingOpen?: () => void }) {
  return <>
    ...
    {page === "Style" && <><div className="fixed-style-heading"><p className="fixed-book-kicker">A practice in detail</p><h1>One Hand,<br /><em>Many Marks</em></h1></div><div className="fixed-style-list"><span>Realism &amp; Portraiture</span><span>Dotwork</span><span>Fine Line</span><span>Blackwork</span><span>Selective Colour</span></div><Photo index={1} className="style-photo-top" /><Photo index={4} className="style-photo-edge" /></>}
    ...
    {page === "FAQ" && <><div className="fixed-book-page-title"><p className="fixed-book-kicker">Before we begin</p><h1>Questions,<br /><em>in pencil.</em></h1></div><div className="fixed-book-faq"><details open><summary>How do I start an enquiry?</summary><p>Share your idea, placement and reference images through the booking page. It starts a conversation, not a confirmed appointment.</p></details><details><summary>What should I include?</summary><p>The subject, style, approximate size, placement and useful visual references.</p></details><details><summary>When can I choose a date?</summary><p>In the local prototype, dates become available after the quote and deposit steps.</p></details><details open><summary>How are price and timing agreed?</summary><p>Abbie reviews the enquiry before preparing an estimate. Final studio wording can be confirmed before launch.</p></details><ReviewNote number={3} className="faq-inline-review" /></div><Photo index={0} className="faq-photo-tucked" /></>}
...
export default function BookExperience() {
  ...
  return <main className={`fixed-book-site book-intro-host is-intro-${introPhase}`}>
    ...
    <article className={`fixed-book ${isOpen ? "is-open" : "is-closed"}`} aria-label="Fletcher Tattoos interactive artist book" aria-hidden={isBookingOpen || undefined} inert={isBookingOpen || undefined}>
      ...
      <button className="fixed-book-front-cover" type="button" onClick={() => setIsOpen(true)} aria-label="Open the Fletcher Tattoos artist book" aria-expanded={isOpen}><span className="fixed-cover-volume">Artist’s book · Volume I</span><img src="/fletcher-tattoos-logo.png" alt="Fletcher Tattoos" /><span className="fixed-cover-rule" aria-hidden="true" /><strong>Open the book</strong><span className="fixed-cover-studio">The Needle Lounge · Shrewsbury</span></button>
